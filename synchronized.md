...
~~~wiki
当新线程进来访问同步代码块的时候，检查锁状态，也就是01标志位，如果锁状态标志位为00，代表目前锁的级别就是轻量级锁，如果是10，代表当前锁级别是重量级锁，如果是01，那么要进行是否是偏斜锁的检查，1代表是偏斜锁，0表示不是偏向锁，如果当前锁不是偏向自己，那么CAS替换线程id，试图将偏斜锁设置为偏向自己的线程id，如果CAS替换失败，这块是没有自旋的，耗时很短，因为失败了，那么需要开始偏向锁撤销，这是等待竞争出现才释放锁的机制，偏向锁的撤销需要等待原持有偏向锁的线程到达安全点，然后暂停原持有偏向锁的线程，这个线程暂停了以后需要看这个线程的活动状态，是否退出了同步代码块，如果原持有偏向锁的线程没有退出同步代码块，那么要开始升级成轻量级锁，
升级轻量级的锁的步骤：
markOop mark = obj->mark();在原来持有偏向锁的线程的栈帧中创建用于存储锁记录的空间
在轻量级锁的整个执行历程中：
有两处CAS操作，只有一处CAS操作会最终走向重量级锁，那就是在CAS替换markword中锁记录指针指向当前线程锁记录，如果失败了，会自旋尝试，当自旋达到一定次数仍然没有成功时，升级为重量级锁
而在轻量级解锁的时候那个CAS操作不管成功与否都会释放锁，这个CAS操作是看对象头的markword中锁记录指针是否仍然指向当前线程锁记录，拷贝在当前线程锁记录的markword信息是否与对象头中的mark word一致，如果这个CAS操作失败，那么会唤醒被挂起的那些线程
~~~
